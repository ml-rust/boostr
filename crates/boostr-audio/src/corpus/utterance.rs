...
use crate::corpus::options::CorpusOptions;
...
pub fn pack_utterances(
    ...
    match opts.pad_to_multiple {
        Some(pad_to_multiple) => Ok(pack_records_padded(vocab, &records, pad_to_multiple)?),
        None => Ok(pack_records(vocab, &records)?),
    }
}
...
pub fn pack_utterances_with_layout(
    ...
    let records = build_records(utterances);
    match opts.pad_to_multiple {
        Some(pad_to_multiple) => Ok(layout.pack_records_padded(&records, pad_to_multiple)?),
        None => Ok(layout.pack_records(&records)?),
    }
}
