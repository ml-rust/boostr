...
//!
//! The espeak-ng backend needs the `g2p` feature and the `espeak-ng` system
//! library at runtime. Without the feature, [`Phonemizer::new`] returns
//! [`G2pError::FeatureDisabled`] so callers can fall back to an alternate G2P.
//! [`Lang`] and [`tokenize_ipa`] have no backend and are always compiled.
//!
//! Mapping phoneme tokens to model ids is the model's business:
//! [`boostr::model::audio::PhonemeVocab`] and
//! [`boostr::model::audio::phonemes_to_ids`] live beside the Kokoro vocab.
...
#[derive(Debug, Error)]
pub enum G2pError {
    #[error("g2p feature is disabled — rebuild boostr-audio with `--features g2p`")]
    FeatureDisabled,
