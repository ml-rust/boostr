...
mod tests {
    ...
    fn test_send_recv_tensor_noop() {
        ...
        let buf = Tensor::<CpuRuntime>::zeros(&[2], DType::F32, &device).unwrap();
    ...
    fn test_broadcast_tensor_noop() {
        ...
        let t = Tensor::<CpuRuntime>::from_slice(&[5.0f32, 10.0], &[2], &device).unwrap();
    ...
    fn test_reduce_scatter_tensor_noop() {
        ...
        let send = Tensor::<CpuRuntime>::from_slice(&[1.0f32, 2.0, 3.0], &[3], &device).unwrap();
        let recv = Tensor::<CpuRuntime>::zeros(&[3], DType::F32, &device).unwrap();
