...
mod tests {
    ...
    impl TrainablePipelineStage<CpuRuntime> for DoubleTrainStage {
        fn forward(&mut self, input: Var<CpuRuntime>) -> Result<Var<CpuRuntime>> {
            ...
            let out =
                Tensor::from_slice(&doubled, input.tensor().shape(), input.tensor().device())?;
            self.saved_count += 1;
    ...
    fn test_schedule_1f1b_single_device() {
        ...
        let mb0 = Tensor::<CpuRuntime>::from_slice(&[1.0f32, 2.0], &[2], &device).unwrap();
        let mb1 = Tensor::<CpuRuntime>::from_slice(&[3.0f32, 4.0], &[2], &device).unwrap();
