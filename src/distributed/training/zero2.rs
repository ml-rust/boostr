...
mod tests {
    ...
    fn test_zero2_single_rank_matches_adamw() {
        ...
        let t1 = Tensor::<CpuRuntime>::from_slice(&[1.0f32, 2.0], &[2], &device).unwrap();
        let t2 = Tensor::<CpuRuntime>::from_slice(&[3.0f32, 4.0], &[2], &device).unwrap();
    ...
    fn test_zero2_step_updates_params() {
        ...
        let id = TensorId::new();
        let t = Tensor::<CpuRuntime>::from_slice(&[5.0f32, 5.0], &[2], &device).unwrap();
