...
mod tests {
    ...
    fn zero_input_stays_zero() {
        ...
        let x = Tensor::<CpuRuntime>::from_slice(&[0.0f32; 6], &[1, 2, 3], &device).unwrap();
        let alpha = Tensor::<CpuRuntime>::from_slice(&[1.0f32, 1.0], &[1, 2, 1], &device).unwrap();
    ...
    fn matches_formula_for_alpha_1() {
        ...
        let x = Tensor::<CpuRuntime>::from_slice(&input, &[1, 1, 5], &device).unwrap();
        let alpha = Tensor::<CpuRuntime>::from_slice(&[1.0f32], &[1, 1, 1], &device).unwrap();
    ...
    fn alpha_scales_frequency() {
        ...
        let x = Tensor::<CpuRuntime>::from_slice(&input, &[1, 1, 2], &device).unwrap();
        let alpha = Tensor::<CpuRuntime>::from_slice(&[2.0f32], &[1, 1, 1], &device).unwrap();
        let y: Vec<f32> = snake(&client, &x, &alpha, 0.0).unwrap().to_vec();
        for (got, x_in) in y.iter().zip(&input) {
    ...
    fn rejects_bad_alpha_shape() {
        ...
        let alpha = Tensor::<CpuRuntime>::from_slice(&[1.0f32, 1.0], &[2], &device).unwrap();
    ...
    fn rejects_bad_input_rank() {
        let (client, device) = cpu_setup();
        let x = Tensor::<CpuRuntime>::from_slice(&[0.0f32; 4], &[2, 2], &device).unwrap();
        let alpha = Tensor::<CpuRuntime>::from_slice(&[1.0f32, 1.0], &[1, 2, 1], &device).unwrap();
        assert!(snake(&client, &x, &alpha, 1e-9).is_err());
    }
}
