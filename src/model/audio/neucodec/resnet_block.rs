...
mod tests {
    ...
    fn group_norm(
        ...
        GroupNorm::new(
            numr::tensor::Tensor::<CpuRuntime>::from_slice(&vec![1.0f32; c], &[c], device).unwrap(),
            numr::tensor::Tensor::<CpuRuntime>::from_slice(&vec![0.0f32; c], &[c], device).unwrap(),
            groups,
            RESNET_NORM_EPS,
            false,
        )
    ...
    fn zero_conv_leaves_residual_unchanged() {
        ...
        let x = Var::new(
            numr::tensor::Tensor::<CpuRuntime>::from_slice(&x_data, &[1, 4, 3], &device).unwrap(),
            false,
        );
        let out = b.forward(&client, &x).unwrap();
    ...
    fn norm_is_group_over_time_not_layernorm_over_channels() {
        ...
        let x = Var::new(
            numr::tensor::Tensor::<CpuRuntime>::from_slice(&x_data, &[1, 2, 4], &device).unwrap(),
            false,
        );
    ...
    fn output_is_finite_for_nonzero_weights() {
        ...
        let x = Var::new(
            numr::tensor::Tensor::<CpuRuntime>::from_slice(&x_data, &[2, c, 5], &device).unwrap(),
            false,
        );
    ...
    fn defaults_to_eval_and_dropout_arms_on_train() {
        ...
        let x = Var::new(
            numr::tensor::Tensor::<CpuRuntime>::from_slice(&x_data, &[1, 4, 8], &device).unwrap(),
            false,
        );
