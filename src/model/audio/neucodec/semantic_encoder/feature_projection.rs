...
mod tests {
    ...
    fn projection(
        ...
        let layer_norm = LayerNorm::new(
            Tensor::<CpuRuntime>::from_slice(&vec![1.0f32; in_dim], &[in_dim], device),
            Tensor::<CpuRuntime>::from_slice(&vec![0.0f32; in_dim], &[in_dim], device),
            1e-5,
            false,
        );
    ...
    fn rejects_wrong_input_width() {
        ...
        let x = Var::new(
            Tensor::<CpuRuntime>::from_slice(&[0.0f32; 5 * 7], &[1, 5, 7], &device),
            false,
        );
