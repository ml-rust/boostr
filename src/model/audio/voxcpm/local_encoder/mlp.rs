//! SwiGLU MLP for `feat_encoder`'s transformer layers.
//!
...
//! [`Linear`] rather than `MaybeQuantLinear`: VoxCPM2's local encoder is
//! dense safetensors, never GGUF-quantized, so that indirection is dropped.
