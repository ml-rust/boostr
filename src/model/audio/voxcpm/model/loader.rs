...
use crate::model::audio::voxcpm::local_dit::{LocalDit, LocalDitConfig};
use crate::model::audio::voxcpm::local_encoder::{LocalEncoder, LocalEncoderConfig};
    ...
    R::Client: TypeConversionOps<R>,
{
    ...
    pub fn from_checkpoint<P: AsRef<Path>, Q: AsRef<Path>>(
        ...
        let config_json = dir.join(DEFAULT_CONFIG_FILE);
        let weights = dir.join(DEFAULT_WEIGHTS_FILE);

        let config = VoxCpm2Config::from_config_json(&config_json)?;
        let base_cfg = MiniCpm4Config::from_config_json(&config_json)?;
        let residual_cfg = MiniCpm4Config::residual_lm_from_config_json(&config_json)?;
        let encoder_cfg = LocalEncoderConfig::from_config_json(&config_json)?;
        let dit_cfg = LocalDitConfig::from_config_json(&config_json)?;
        let fsq_cfg = FsqConfig::from_config_json(&config_json)?;

        let vae = audiovae_path.as_ref();
        Ok(Self {
            vae_encoder: AudioVaeEncoder::from_safetensors(vae, device)?,
            vae_decoder: AudioVaeDecoder::from_safetensors(vae, device)?,
            feat_encoder: LocalEncoder::from_safetensors(&weights, encoder_cfg, device, dtype)?,
            base_lm: MiniCpm4Model::from_safetensors_with(
                &weights,
            ...
            residual_lm: MiniCpm4Model::from_safetensors_with(
                &weights,
            ...
            feat_decoder: LocalDit::from_safetensors(&weights, dit_cfg, device, dtype)?,
            fsq: ScalarQuantization::from_safetensors(&weights, fsq_cfg, device, dtype)?,
            aux: AuxProjections::from_safetensors(&weights, fsq_cfg, device, dtype)?,
            config,
        })
    }
}
