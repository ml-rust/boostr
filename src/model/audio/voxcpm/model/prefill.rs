...
mod tests {
    ...
    //! `SequenceLayout`'s own tests in `sequence.rs`. This module pins the
    //! `ref_feat = None` branch specifically, since `prefill_capturing(...,
    //! None, ...)` is the exact call the training loop makes for a manifest
    //! row with no `ref_wav` (see `eval_common.rs`'s doc comment on
