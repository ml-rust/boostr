...
use numr::ops::{ActivationOps, ConvOps, IndexingOps, NormalizationOps, UnaryOps};
    ...
    R::Client: IndexingOps<R>,
{
    ...
    pub fn load(config: &UniversalConfig, vb: &mut VarBuilder<R>) -> Result<Self> {
        match config.model_type.as_str() {
            ...
            "hybrid" => Err(Error::ModelError {
                reason: "Hybrid model loading not yet implemented in new boostr".into(),
            }),
    ...
    pub fn needs_kv_cache(&self) -> bool {
        matches!(self, LoadedModel::Llama(_))
    }

    /// Whether this model uses SSM state.
    pub fn needs_ssm_state(&self) -> bool {
        matches!(self, LoadedModel::Mamba2(_))
    }
    ...
    /// Get the Mamba2 config (for SSM state allocation). Returns None for non-Mamba2 models.
