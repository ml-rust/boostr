...
//! and [`LoadedModel::forward_qwen35`].
    ...
    R::Client: IndexingOps<R>,
{
    ...
    /// state (linear-attention layers). `position` must equal
    /// `kv_cache.seq_len()`.
        ...
        R::Client: ModelClient<R> + BinaryOps<R> + ShapeOps<R> + TensorOps<R> + ScalarOps<R>,
    {
        ...
        match self {
            LoadedModel::Qwen35(m) => {
                m.forward_qwen35(&client, input_ids, kv_cache, gdn_state, position)
            }
            LoadedModel::Llama(_) | LoadedModel::LlamaTp(_) => Err(Error::ModelError {
            ...
            LoadedModel::Multimodal(m) => m
                .llm()
                .forward_qwen35(input_ids, kv_cache, gdn_state, position),
        }
    }
}
