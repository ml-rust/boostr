...
mod tests {
    ...
    fn forward_shape_is_preserved() {
        ...
        let x =
            Tensor::<CpuRuntime>::from_slice(&[0.5f32; 2 * 4 * 6], &[2, 4, 6], &device).unwrap();
        let gamma = Tensor::<CpuRuntime>::from_slice(&[1.0f32; 2 * 4], &[2, 4], &device).unwrap();
        let beta = Tensor::<CpuRuntime>::from_slice(&[0.0f32; 2 * 4], &[2, 4], &device).unwrap();
    ...
    fn unit_style_preserves_normalized_stats() {
        ...
        let gamma = Tensor::<CpuRuntime>::from_slice(&[1.0f32; 2], &[1, 2], &device).unwrap();
        let beta = Tensor::<CpuRuntime>::from_slice(&[0.0f32; 2], &[1, 2], &device).unwrap();
    ...
    fn style_rescales_and_shifts() {
        ...
        let x = Tensor::<CpuRuntime>::from_slice(&[-1.0f32, 0.0, 1.0, 2.0], &[1, 1, 4], &device)
            .unwrap();
        let gamma = Tensor::<CpuRuntime>::from_slice(&[2.0f32], &[1, 1], &device).unwrap();
        let beta = Tensor::<CpuRuntime>::from_slice(&[3.0f32], &[1, 1], &device).unwrap();
    ...
    fn rejects_wrong_input_rank() {
        let (client, device) = cpu_setup();
        let adain = AdaIn1d::new(4, 1e-5);
        let x = Tensor::<CpuRuntime>::from_slice(&[0.0f32; 8], &[2, 4], &device).unwrap();
        let gamma = Tensor::<CpuRuntime>::from_slice(&[1.0f32; 8], &[2, 4], &device).unwrap();
        let beta = Tensor::<CpuRuntime>::from_slice(&[0.0f32; 8], &[2, 4], &device).unwrap();
    ...
    fn rejects_mismatched_channel_count() {
        let (client, device) = cpu_setup();
        let adain = AdaIn1d::new(4, 1e-5);
        let x = Tensor::<CpuRuntime>::from_slice(&[0.0f32; 24], &[2, 3, 4], &device).unwrap();
        let gamma = Tensor::<CpuRuntime>::from_slice(&[1.0f32; 6], &[2, 3], &device).unwrap();
        let beta = Tensor::<CpuRuntime>::from_slice(&[0.0f32; 6], &[2, 3], &device).unwrap();
        assert!(adain.forward(&client, &x, &gamma, &beta).is_err());
    }
}
