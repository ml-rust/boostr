...
mod tests {
    ...
    fn test_conv2d_output_shape() {
        ...
        let weight =
            Tensor::<CpuRuntime>::from_slice(&[0.1f32; 108], &[4, 3, 3, 3], &device).unwrap();
        let conv = Conv2d::new(weight, None, (1, 1), PaddingMode::Valid, (1, 1), 1, false);

        // input: [batch=2, channels=3, height=8, width=10]
        let input = Var::new(
            Tensor::<CpuRuntime>::from_slice(&[0.1f32; 480], &[2, 3, 8, 10], &device).unwrap(),
            false,
        );
    ...
    fn test_conv2d_with_bias() {
        ...
        let weight = Tensor::<CpuRuntime>::from_slice(&[2.0f32], &[1, 1, 1, 1], &device).unwrap();
        ...
        let input = Var::new(
            Tensor::<CpuRuntime>::from_slice(&[3.0f32, 5.0], &[1, 1, 1, 2], &device).unwrap(),
            false,
        );
        let out = conv.forward(&client, &input).unwrap();
