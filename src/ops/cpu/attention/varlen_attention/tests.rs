...
fn test_varlen_fwd_shape() {
    ...
    let q =
        Tensor::<CpuRuntime>::from_slice(&q_data, &[total_q, num_heads, head_dim], &dev).unwrap();
    let k =
        Tensor::<CpuRuntime>::from_slice(&k_data, &[total_q, num_heads, head_dim], &dev).unwrap();
    let v =
        Tensor::<CpuRuntime>::from_slice(&v_data, &[total_q, num_heads, head_dim], &dev).unwrap();
...
fn test_varlen_bwd_shapes() {
    ...
    let dout =
        Tensor::<CpuRuntime>::from_slice(&do_data, &[total_q, num_heads, head_dim], &dev).unwrap();
