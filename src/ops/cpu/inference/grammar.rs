...
mod tests {
    ...
    fn test_grammar_dfa_mask_basic() {
        ...
        let transition_table_tensor =
            Tensor::from_slice(&transition_table, &[num_states * 256], &device).unwrap();
        let accepting_mask_tensor =
            Tensor::from_slice(&accepting_mask, &[num_states], &device).unwrap();
        let vocab_bytes_tensor =
            Tensor::from_slice(&vocab_bytes_data, &[vocab_bytes_data.len()], &device).unwrap();
        let vocab_offsets_tensor =
            Tensor::from_slice(&vocab_offsets_data, &[vocab_offsets_data.len()], &device).unwrap();
        ...
        let logits = Tensor::from_slice(&[1.0f32, 2.0, 3.0], &[3], &device).unwrap();
