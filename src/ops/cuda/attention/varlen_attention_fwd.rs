...
pub(super) fn varlen_attention_fwd_impl(
    client: &CudaClient,
    ...
    varlen_attention_fwd_impl_inner(
        client,
        q,
        k,
        v,
...
/// Test-only entry point: run varlen attention forward with an explicit
...
#[doc(hidden)]
#[allow(clippy::too_many_arguments)]
pub fn varlen_attention_fwd_with_tile_for_test(
    client: &CudaClient,
    q: &Tensor<CudaRuntime>,
    k: &Tensor<CudaRuntime>,
    v: &Tensor<CudaRuntime>,
...
#[allow(clippy::too_many_arguments)]
fn varlen_attention_fwd_impl_inner(
    client: &CudaClient,
    q: &Tensor<CudaRuntime>,
    k: &Tensor<CudaRuntime>,
    v: &Tensor<CudaRuntime>,
    ...
    // Block sizes are chosen so shared memory fits this device's opt-in limit
    // (after set_smem_attribute opts in to larger dynamic smem). Sized on the
    // backward requirement so a given (head_dim, dtype) call site's fwd and
    // bwd agree on one tile — see `varlen_attention_block_config::block_config_with_override`.
    let (block_m, block_n, variant) = block_config_with_override(head_dim, dtype, force_large)?;
    let tile_suffix = variant.suffix();
    let kernel_name = format!("varlen_flash_attention_fwd_{head_dim}_{dtype_suffix}{tile_suffix}");
