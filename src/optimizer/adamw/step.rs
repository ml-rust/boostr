//! The AdamW update itself, plus the `Optimizer` trait bridge.
//!
//! The per-parameter update math (multi-tensor fast path and the widened,
//! master-copy path) lives in `super::apply`.

use super::types::AdamW;
...
impl<R: Runtime<DType = DType>> AdamW<R> {
            ...
            + FusedOptimizerOps<R>,
    {
        ...
        let (direct, widened) = self.collect_and_init_state(client, params, grads)?;

        self.apply_direct(
            client, params, grads, &direct, lr, beta1, beta2, eps, wd, step_size,
        )?;
        self.apply_widened(
            client, params, grads, widened, lr, beta1, beta2, eps, wd, step_size,
        )?;
