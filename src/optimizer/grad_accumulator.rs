...
mod tests {
    ...
    fn test_accumulate_returns_averaged_grads() {
        ...
        let t1 = Tensor::<CpuRuntime>::from_slice(&[2.0f32, 4.0], &[2], &device).unwrap();
        ...
        let t2 = Tensor::<CpuRuntime>::from_slice(&[6.0f32, 8.0], &[2], &device).unwrap();
    ...
    fn test_accumulate_resets_after_full() {
        ...
        for _ in 0..2 {
            let t = Tensor::<CpuRuntime>::from_slice(&[1.0f32], &[1], &device).unwrap();
    ...
    fn test_accumulate_single_step() {
        ...
        let id = numr::tensor::TensorId::new();
        let t = Tensor::<CpuRuntime>::from_slice(&[3.0f32, 5.0], &[2], &device).unwrap();
        let mut g = GradStore::new();
        g.insert(id, t);
