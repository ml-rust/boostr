//! Super-block quantization: derives ONE super-scale per block from the
//! per-group fits in [`super::fit`], per [`SuperPrecision`]'s storage rule,
//! then re-derives every group's codes against the final effective scale.
//! This is the module's public entry point.

use half::{bf16, f16};

use super::fit::{
    CodeGrid, FULL_64, GroupFit, RESERVED_63, derive_codes, fit_group_scale, refine_sub_scale,
};
...
pub(in crate::quant::codebook) fn round_bf16(x: f32) -> f32 {
    bf16::from_f32(x).to_f32()
}

/// Rounds `x` through `f16` and back to `f32`. Shared the same way as
/// [`round_bf16`].
pub(in crate::quant::codebook) fn round_f16(x: f32) -> f32 {
...
fn quantize_super_block(values: &[f32], weights: &[f32], precision: SuperPrecision) -> Vec<f32> {
    ...
    for ((group, weight_group), fit) in groups.iter().zip(weight_groups.iter()).zip(fits.iter()) {
        ...
        let sub = refine_sub_scale(group, weight_group, rounded, grid, MAX_SUB, |s| {
...
/// [`super::super::roundtrip::codebook_round_trip`] — same tail handling (a
