...
// Layout: [d:f16(2), scales:6B, qs:32B, qh:16B]
// 3-bit packed scales, same base-3 grid decode as IQ1_S
// but with per-sub-block scale
...
extern "C" __global__ __launch_bounds__(256, 1) void quant_gemv_iq1_m_f32(
    ...
    for (unsigned int b = lane; b < blocks_per_row; b += WARP_SIZE) {
        const unsigned char* block = w_row + b * IQ1_M_BLOCK_BYTES;
        __half d_half;
        ...
        unsigned int base = b * IQ1_M_BLOCK_SIZE;

        for (int sb = 0; sb < 16; sb++) {
            // Extract 3-bit scale from packed scales_data (6 bytes, 16 x 3-bit)
