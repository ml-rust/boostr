...
// ============================================================================
// Token-batched IQ2_S GEMV with dp4a (IQ2_S weight x Q8_1 activation)
//
// One block covers NTOK consecutive token columns and decodes each 8-element
// sub-group — one 1024-point grid read, one explicit sign byte and one scale
...
// ragged-tail rule. Every field read here is a single byte, so `load_int_ua`
...
extern "C" __global__ __launch_bounds__(mwr_nwarps_ntok(2) * WARP_SIZE, 1) void quant_gemv_iq2_s_q8_1_mwr_n2(
    const unsigned char* __restrict__ q8_act,
    const unsigned char* __restrict__ weight,
    float* __restrict__ output,
    unsigned int M, unsigned int K, unsigned int N
) {
    quant_gemv_iq_grid_q8_1_mwr_ntok<IqGridIq2S, 2>(q8_act, weight, output, M, K, N);
}

extern "C" __global__ __launch_bounds__(mwr_nwarps_ntok(4) * WARP_SIZE, 1) void quant_gemv_iq2_s_q8_1_mwr_n4(
    const unsigned char* __restrict__ q8_act,
    const unsigned char* __restrict__ weight,
    float* __restrict__ output,
    unsigned int M, unsigned int K, unsigned int N
) {
    quant_gemv_iq_grid_q8_1_mwr_ntok<IqGridIq2S, 4>(q8_act, weight, output, M, K, N);
}
