...
// ============================================================================
// Token-batched IQ2_XXS GEMV with dp4a (IQ2_XXS weight x Q8_1 activation)
//
// One block covers NTOK consecutive token columns and decodes each 8-element
// sub-group — one grid read, one sign-table read and one scale unpack — ONCE
...
// grid-indexed IQ formats; see the header for the lane map, the ragged-tail
// rule and the alignment constraint that puts this format's two 4-byte `qs`
// reads through `load_int_ua`.
//
...
extern "C" __global__ __launch_bounds__(mwr_nwarps_ntok(2) * WARP_SIZE, 1) void quant_gemv_iq2_xxs_q8_1_mwr_n2(
    const unsigned char* __restrict__ q8_act,
    const unsigned char* __restrict__ weight,
    float* __restrict__ output,
    unsigned int M, unsigned int K, unsigned int N
) {
    quant_gemv_iq_grid_q8_1_mwr_ntok<IqGridIq2Xxs, 2>(q8_act, weight, output, M, K, N);
}

extern "C" __global__ __launch_bounds__(mwr_nwarps_ntok(4) * WARP_SIZE, 1) void quant_gemv_iq2_xxs_q8_1_mwr_n4(
    const unsigned char* __restrict__ q8_act,
    const unsigned char* __restrict__ weight,
    float* __restrict__ output,
    unsigned int M, unsigned int K, unsigned int N
) {
    quant_gemv_iq_grid_q8_1_mwr_ntok<IqGridIq2Xxs, 4>(q8_act, weight, output, M, K, N);
}
