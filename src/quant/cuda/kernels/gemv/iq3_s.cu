// IQ3_S GEMV kernel — F32 activation path
//
// IQ3_S block: 256 elements, 110 bytes.
//
// IQ3_S is a codebook quantization: `qs` holds INDICES into a grid of
...
extern "C" __global__ __launch_bounds__(256, 1) void quant_gemv_iq3_s_f32(
    ...
    unsigned int blocks_per_row = K / IQ3_S_BLOCK_SIZE;
    unsigned int row_bytes = blocks_per_row * IQ3_S_BLOCK_BYTES;
    const float* act_row = activation + row * K;
    const unsigned char* w_row = weight + col * row_bytes;

    float w[IQ3_S_BLOCK_SIZE];
    float sum = 0.0f;
    for (unsigned int b = lane; b < blocks_per_row; b += WARP_SIZE) {
        iq3_s_dequant_block(w_row + (unsigned long long)b * IQ3_S_BLOCK_BYTES, w);
        const float* act = act_row + (unsigned long long)b * IQ3_S_BLOCK_SIZE;
        for (int k = 0; k < IQ3_S_BLOCK_SIZE; k++)
            sum += act[k] * w[k];
    }
...
// ============================================================================
// Token-batched IQ3_S GEMV with dp4a (IQ3_S weight x Q8_1 activation)
//
// One block covers NTOK consecutive token columns and decodes each 8-element
// sub-group — TWO nine-bit grid reads, one explicit sign byte and one scale
// nibble — ONCE for all of them, instead of repeating that per token as the
// F32 kernel above does.
...
// not apply to it.
//
// SCALE INDEXING. IQ3_S packs its `scales` field exactly as IQ2_XS does, two
// 4-bit values per byte, but indexes it per THIRTY-TWO elements rather than per
// sixteen, and its value is `d * (1 + 2 * s)` rather than `d * (0.5 + s) * 0.25`.
// The policy in the header follows `dequant_iq3_s`'s index expression, not the
// packing.
//
...
extern "C" __global__ __launch_bounds__(mwr_nwarps_ntok(2) * WARP_SIZE, 1) void quant_gemv_iq3_s_q8_1_mwr_n2(
    const unsigned char* __restrict__ q8_act,
    const unsigned char* __restrict__ weight,
    float* __restrict__ output,
    unsigned int M, unsigned int K, unsigned int N
) {
    quant_gemv_iq_grid_q8_1_mwr_ntok<IqGridIq3S, 2>(q8_act, weight, output, M, K, N);
}

extern "C" __global__ __launch_bounds__(mwr_nwarps_ntok(4) * WARP_SIZE, 1) void quant_gemv_iq3_s_q8_1_mwr_n4(
    const unsigned char* __restrict__ q8_act,
    const unsigned char* __restrict__ weight,
    float* __restrict__ output,
    unsigned int M, unsigned int K, unsigned int N
) {
    quant_gemv_iq_grid_q8_1_mwr_ntok<IqGridIq3S, 4>(q8_act, weight, output, M, K, N);
}
