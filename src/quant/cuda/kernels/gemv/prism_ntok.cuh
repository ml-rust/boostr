...
//
// The expansions below port `vec_dot_q1_0_q8_1` and `vec_dot_q2_0_q8_1` from
// the fork's `ggml-cuda/vecdotq.cuh` — same LUT words, same `__byte_perm`
// selectors. The fork feeds each expansion a 16-bit word covering 8 (Q2_0)
// or 16 (Q1_0) consecutive elements; here a lane owns elements `4w..4w+3`
// and `4w+16..4w+19` of its chunk, so the 16-bit (Q2_0) or 8-bit (Q1_0)
// input is assembled from those two positions first. The permutes then land
// the low half in `v_lo` and the high half in `v_hi`, which is what the body
