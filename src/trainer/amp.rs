...
mod tests {
    ...
    fn test_amp_trainer_loss_scale() {
        ...
        let w = Tensor::<CpuRuntime>::from_slice(&[1.0f32], &[1], &device).unwrap();
    ...
    fn test_per_layer_precision_policy() {
        ...
        let w1 = Tensor::<CpuRuntime>::from_slice(&[1.0f32, 2.0], &[2], &device).unwrap();
        let w2 = Tensor::<CpuRuntime>::from_slice(&[3.0f32, 4.0], &[2], &device).unwrap();
        let id1 = w1.id();
        let id2 = w2.id();
    ...
    fn test_fp32_override_set_bulk() {
        ...
        let w1 = Tensor::<CpuRuntime>::from_slice(&[1.0f32], &[1], &device).unwrap();
        let w2 = Tensor::<CpuRuntime>::from_slice(&[2.0f32], &[1], &device).unwrap();
        let w3 = Tensor::<CpuRuntime>::from_slice(&[3.0f32], &[1], &device).unwrap();
        let id1 = w1.id();
        let id2 = w2.id();
        let id3 = w3.id();
    ...
    fn test_amp_trainer_master_params_are_f32() {
        ...
        let w = Tensor::<CpuRuntime>::from_slice(&[1.0f32, 2.0], &[2], &device).unwrap();
        let mut params = HashMap::new();
        let id = w.id();
        params.insert(id, w);
