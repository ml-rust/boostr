...
fn test_mla_scaled_dot_product_attention_parity() {
    ...
    with_cuda_backend(|cuda_client, cuda_device| {
        ...
        let q_c = Var::<CudaRuntime>::new(
            Tensor::from_slice(&q_data.to_vec::<f32>(), &[b, h, s, d_k], &cuda_device).unwrap(),
            false,
        );
        let k_c = Var::<CudaRuntime>::new(
            Tensor::from_slice(&k_data.to_vec::<f32>(), &[b, h, s, d_k], &cuda_device).unwrap(),
            false,
        );
        let v_c = Var::<CudaRuntime>::new(
            Tensor::from_slice(&v_data.to_vec::<f32>(), &[b, h, s, d_v], &cuda_device).unwrap(),
            false,
        );
    ...
    with_wgpu_backend(|wgpu_client, wgpu_device| {
        ...
        let q_w = Var::<WgpuRuntime>::new(
            Tensor::from_slice(&q_data.to_vec::<f32>(), &[b, h, s, d_k], &wgpu_device).unwrap(),
            false,
        );
        let k_w = Var::<WgpuRuntime>::new(
            Tensor::from_slice(&k_data.to_vec::<f32>(), &[b, h, s, d_k], &wgpu_device).unwrap(),
            false,
        );
        let v_w = Var::<WgpuRuntime>::new(
            Tensor::from_slice(&v_data.to_vec::<f32>(), &[b, h, s, d_v], &wgpu_device).unwrap(),
            false,
        );
...
fn test_mla_forward_no_q_lora() {
    ...
    let x = Var::new(
        Tensor::<CpuRuntime>::from_slice(&[0.1f32; 4 * 16], &[1, 4, 16], &cpu_device).unwrap(),
        false,
    );
...
fn test_mla_forward_different_head_dim_v() {
    ...
    let mla = Mla::<CpuRuntime>::from_config(&cfg, &cpu_device).unwrap();
    let x = Var::new(
        Tensor::<CpuRuntime>::from_slice(&[0.1f32; 3 * 16], &[1, 3, 16], &cpu_device).unwrap(),
        false,
    );
    let out = mla.forward(&cpu_client, &x).unwrap();
...
fn test_mla_output_finite() {
    ...
    let mla = Mla::<CpuRuntime>::from_config(&cfg, &cpu_device).unwrap();
    let x = Var::new(
        Tensor::<CpuRuntime>::from_slice(&[0.1f32; 2 * 8], &[1, 2, 8], &cpu_device).unwrap(),
        false,
    );
    let out = mla.forward(&cpu_client, &x).unwrap();
