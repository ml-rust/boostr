...
fn test_moe_permute_tokens_parity() {
    ...
    let indices =
        Tensor::<CpuRuntime>::from_slice(&indices_data, &[num_tokens, k], &cpu_device).unwrap();
    ...
    with_cuda_backend(|cuda_client, cuda_device| {
        ...
        .unwrap();
        let indices_c = Tensor::from_slice(&indices_data, &[num_tokens, k], &cuda_device).unwrap();
    ...
    with_wgpu_backend(|wgpu_client, wgpu_device| {
        ...
        .unwrap();
        let indices_w = Tensor::from_slice(&indices_data, &[num_tokens, k], &wgpu_device).unwrap();
...
fn test_moe_unpermute_tokens_parity() {
    ...
    let weights =
        Tensor::<CpuRuntime>::from_slice(&weights_data, &[num_tokens, k], &cpu_device).unwrap();
    ...
    with_cuda_backend(|cuda_client, cuda_device| {
        ...
        .unwrap();
        let indices_c = Tensor::from_slice(&indices_data, &[num_tokens, k], &cuda_device).unwrap();
        let weights_c = Tensor::from_slice(&weights_data, &[num_tokens, k], &cuda_device).unwrap();
    ...
    with_wgpu_backend(|wgpu_client, wgpu_device| {
        ...
        .unwrap();
        let indices_w = Tensor::from_slice(&indices_data, &[num_tokens, k], &wgpu_device).unwrap();
        let weights_w = Tensor::from_slice(&weights_data, &[num_tokens, k], &wgpu_device).unwrap();
...
fn test_moe_grouped_gemm_parity() {
    ...
    let offsets =
        Tensor::<CpuRuntime>::from_slice(&offsets_data, &[num_experts + 1], &cpu_device).unwrap();
    ...
    with_cuda_backend(|cuda_client, cuda_device| {
        ...
        .unwrap();
        let offsets_c =
            Tensor::from_slice(&offsets_data, &[num_experts + 1], &cuda_device).unwrap();
        let result_c = cuda_client
    ...
    with_wgpu_backend(|wgpu_client, wgpu_device| {
        ...
        .unwrap();
        let offsets_w =
            Tensor::from_slice(&offsets_data, &[num_experts + 1], &wgpu_device).unwrap();
        let result_w = wgpu_client
