...
fn test_apply_rope_parity() {
    ...
    with_cuda_backend(|cuda_client, cuda_device| {
        ...
        let x_c = Var::<CudaRuntime>::new(
            Tensor::from_slice(&x_data.to_vec::<f32>(), &[b, h, s, d], &cuda_device).unwrap(),
            false,
        );
        let cos_c = Var::<CudaRuntime>::new(
            Tensor::from_slice(&cos_data.to_vec::<f32>(), &[s, d / 2], &cuda_device).unwrap(),
            false,
        );
        let sin_c = Var::<CudaRuntime>::new(
            Tensor::from_slice(&sin_data.to_vec::<f32>(), &[s, d / 2], &cuda_device).unwrap(),
            false,
        );
    ...
    with_wgpu_backend(|wgpu_client, wgpu_device| {
        ...
        let x_w = Var::<WgpuRuntime>::new(
            Tensor::from_slice(&x_data.to_vec::<f32>(), &[b, h, s, d], &wgpu_device).unwrap(),
            false,
        );
        let cos_w = Var::<WgpuRuntime>::new(
            Tensor::from_slice(&cos_data.to_vec::<f32>(), &[s, d / 2], &wgpu_device).unwrap(),
            false,
        );
        let sin_w = Var::<WgpuRuntime>::new(
            Tensor::from_slice(&sin_data.to_vec::<f32>(), &[s, d / 2], &wgpu_device).unwrap(),
            false,
        );
...
fn test_interleaved_vs_standard_different_outputs() {
    ...
    let cos = Var::<CpuRuntime>::new(
        numr::tensor::Tensor::from_slice(&cos_data, &[s, d / 2], &cpu_device).unwrap(),
        false,
    );
    let sin = Var::<CpuRuntime>::new(
        numr::tensor::Tensor::from_slice(&sin_data, &[s, d / 2], &cpu_device).unwrap(),
        false,
    );
