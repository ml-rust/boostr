...
fn test_packed_matches_standard_single_sequence() {
    ...
    let x_standard = Var::<CpuRuntime>::new(
        Tensor::<CpuRuntime>::from_slice(&x_4d_data, &[1, h, s, d], &cpu_device).unwrap(),
        false,
    );
...
fn test_packed_position_reset_parity() {
    ...
    let cos = Var::<CpuRuntime>::new(
        Tensor::<CpuRuntime>::from_slice(&cos_data, &[max_seq, half_d], &cpu_device).unwrap(),
        false,
    );
    let sin = Var::<CpuRuntime>::new(
        Tensor::<CpuRuntime>::from_slice(&sin_data, &[max_seq, half_d], &cpu_device).unwrap(),
        false,
    );
    ...
    let x = Var::<CpuRuntime>::new(
        Tensor::<CpuRuntime>::from_slice(&x_data, &[4, h, d], &cpu_device).unwrap(),
        false,
    );
    let pids = Tensor::<CpuRuntime>::from_slice(&[0i32, 1, 0, 1], &[4], &cpu_device).unwrap();
