...
fn make_encoder(
    ...
    let encoder = Encoder::from_weights(config, Pooling::Mean, |name| match name {
        "embeddings.word_embeddings.weight" => {
            Ok(Tensor::from_slice(&tok_emb, &[vocab, hidden], d).unwrap())
        }
        "embeddings.position_embeddings.weight" => {
            Ok(Tensor::from_slice(&pos_emb, &[max_pos, hidden], d).unwrap())
        }
        "embeddings.layer_norm.weight" => Ok(Tensor::from_slice(&[1.0f32; 8], &[8], d).unwrap()),
        ...
        {
            Ok(Tensor::from_slice(&vec![0.02f32; hidden * hidden], &[hidden, hidden], d).unwrap())
        }
        ...
        n if n.ends_with("intermediate.dense.weight") => {
            Ok(Tensor::from_slice(&vec![0.02f32; inter * hidden], &[inter, hidden], d).unwrap())
        }
        n if n.ends_with("intermediate.dense.bias") => {
            Ok(Tensor::from_slice(&[0.0f32; 16], &[16], d).unwrap())
        }
        n if n.ends_with("output.dense.weight") => {
            Ok(Tensor::from_slice(&vec![0.02f32; hidden * inter], &[hidden, inter], d).unwrap())
        }
...
fn embed_padded(
    ...
    let input = Tensor::<CpuRuntime>::from_slice(&flat, &[batch, max_len], device).unwrap();
    let mask = Tensor::<CpuRuntime>::from_slice(&mask_flat, &[batch, max_len], device).unwrap();
    let pooled = encoder
...
fn embed_varlen(
    ...
    let d = device;
    let input_t = Tensor::<CpuRuntime>::from_slice(&flat_ids, &[total], d).unwrap();
    let cu_t = Tensor::<CpuRuntime>::from_slice(&cu, &[batch + 1], d).unwrap();
    let pos_t = Tensor::<CpuRuntime>::from_slice(&pos_ids, &[total], d).unwrap();
    let seg_t = Tensor::<CpuRuntime>::from_slice(&seg_ids, &[total], d).unwrap();
