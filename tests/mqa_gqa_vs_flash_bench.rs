...
fn run_shape(client: &CudaClient, device: &CudaDevice, shape: &Shape) {
    ...
    let q = Tensor::<CudaRuntime>::from_slice(&q_data, &q_shape, device);
    let k = Tensor::<CudaRuntime>::from_slice(&k_data, &kv_shape, device);
    let v = Tensor::<CudaRuntime>::from_slice(&v_data, &kv_shape, device);
    let dout = Tensor::<CudaRuntime>::from_slice(&dout_data, &q_shape, device);
