...
fn listed_nn_modules_preserve_explicit_parameter_ids() {
    ...
    let linear_w = Tensor::<CpuRuntime>::from_slice(&[1.0f32; 6], &[2, 3], &device).unwrap();
    let linear_w_id = linear_w.id();
    let linear_b = Tensor::<CpuRuntime>::from_slice(&[0.0f32; 2], &[2], &device).unwrap();
    ...
    let embedding_w = Tensor::<CpuRuntime>::from_slice(&[1.0f32; 12], &[3, 4], &device).unwrap();
    ...
    let layernorm_w = Tensor::<CpuRuntime>::from_slice(&[1.0f32; 4], &[4], &device).unwrap();
    let layernorm_w_id = layernorm_w.id();
    let layernorm_b = Tensor::<CpuRuntime>::from_slice(&[0.0f32; 4], &[4], &device).unwrap();
    ...
    let rms_w = Tensor::<CpuRuntime>::from_slice(&[1.0f32; 4], &[4], &device).unwrap();
    let rms_w_id = rms_w.id();
    let rms = RmsNorm::with_id(rms_w.clone(), rms_w_id, 1e-5, true);
    assert_eq!(rms.parameters()[0].0, rms_w_id);

    let conv_w = Tensor::<CpuRuntime>::from_slice(&[0.1f32; 3], &[1, 1, 3], &device).unwrap();
    let conv_w_id = conv_w.id();
    let conv_b = Tensor::<CpuRuntime>::from_slice(&[0.0f32], &[1], &device).unwrap();
...
fn nn_module_adamw_loop_preserves_state_and_converges() {
    ...
    let input = Tensor::<CpuRuntime>::from_slice(&[1.0f32], &[1, 1], &device).unwrap();
    let target = Tensor::<CpuRuntime>::from_slice(&[1.0f32], &[1, 1], &device).unwrap();

    let weight_init = Tensor::<CpuRuntime>::from_slice(&[0.0f32], &[1, 1], &device).unwrap();
    let weight_id = weight_init.id();
    let bias_init = Tensor::<CpuRuntime>::from_slice(&[0.0f32], &[1], &device).unwrap();
