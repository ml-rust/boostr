...
fn make_encoder(device: &CudaDevice, client: &CudaClient) -> Encoder<CudaRuntime> {
    ...
    let ones = |n: usize| {
        Ok(Weight::Standard(
            Tensor::<CudaRuntime>::from_slice(&vec![1.0f32; n], &[n], d).unwrap(),
        ))
    };
    let zeros = |n: usize| {
        Ok(Weight::Standard(
            Tensor::<CudaRuntime>::from_slice(&vec![0.0f32; n], &[n], d).unwrap(),
        ))
    };
...
fn run_batch(
    ...
    let total = ids.len();
    let input = Tensor::<CudaRuntime>::from_slice(&ids, &[total], device).unwrap();
    let cu_t = Tensor::<CudaRuntime>::from_slice(&cu, &[n_docs + 1], device).unwrap();
    let pos_t = Tensor::<CudaRuntime>::from_slice(&pos, &[total], device).unwrap();
    let seg_t = Tensor::<CudaRuntime>::from_slice(&seg, &[total], device).unwrap();
...
fn single_short_query_padded_path() {
    ...
    for seq in [1usize, 7, 16, 17, 31, 33, 64, 65] {
        ...
        let input = Tensor::<CudaRuntime>::from_slice(&ids, &[1, seq], &device).unwrap();
        let out = encoder
